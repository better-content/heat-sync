...
/**
 * Derived from Create 6.0.8's Encased Fan processing Ponder scene under Create's MIT code license.
 * The blasting segment uses Better Content's active-burner policy; all other segments remain upstream behavior.
 */
public final class ActiveFanProcessingPonderScene {

	private ActiveFanProcessingPonderScene() {}
	...
	public static void processing(SceneBuilder builder, SceneBuildingUtil util) {
		...
		scene.world().setBlock(blockPos, AllBlocks.BLAZE_BURNER.getDefaultState()
		...
		scene.overlay().showOutlineWithText(util.select().fromTo(blockPos, blockPos.west(2)), 80)
			...
			.text("When passing through an actively fueled Blaze Burner, the Air Flow becomes Heated");
		scene.idle(80);
